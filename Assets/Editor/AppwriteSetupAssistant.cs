...
namespace Appwrite.Editor
{
    ...
    {
        private const string UNITASK_PACKAGE_URL = "https://github.com/Cysharp/UniTask.git?path=src/UniTask/Assets/Plugins/UniTask#f213ff497e4ff462a77319cf677cf20cc0860ca9";
        private const string UNITASK_PACKAGE_NAME = "com.cysharp.unitask";
        private const string WEBSOCKET_PACKAGE_URL = "https://github.com/endel/NativeWebSocket.git#1d8b49b3fee41c09a98141f1f1a5e4db47e14229";
        ...
        public static void RefreshPackageStatus(Action onRefreshed = null)
        {
            ...
            void CheckProgress()
            {
                ...
                {
                    Debug.LogWarning($"Appwrite Setup: Could not refresh package status - {request.Error?.message ?? "Unknown"}");
                }
                _isBusy = false;
        ...
        private static void InstallNextPackage(Queue<string> packageQueue, Action onSuccess, Action<string> onError)
        {
            ...
            void CheckInstallProgress()
            {
                ...
                if (request.Status == StatusCode.Success)
                {
                    Debug.Log($"Appwrite Setup: Successfully installed {request.Result.displayName}.");
                ...
                else
                {
                    ...
                    Debug.LogError($"Appwrite Setup: Failed to install {packageUrl} - {error}");
                    AssetDatabase.StopAssetEditing();
                    _isBusy = false;
        ...
        public static void ResetSetup()
        {
            ...
            HasUniTask = false;
            HasWebSocket = false;
            Debug.Log("Appwrite Setup: Setup state reset. Reopening the window will trigger the check.");
        }
    }
}
