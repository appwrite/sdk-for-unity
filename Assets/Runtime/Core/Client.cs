    ...
    public class Client
    {
        ...
        private Client()
        {
            ...
            _headers = new Dictionary<string, string>()
            {
                ...
                { "user-agent" , $"AppwriteUnitySDK/0.1.0 ({Environment.OSVersion.Platform}; {Environment.OSVersion.VersionString})"},
                { "x-sdk-name", "Unity" },
            ...
            bool selfSigned = false)
        {
            ...
            Configure(client, endpoint, projectId, endpointRealtime, locale, selfSigned);
            return client;
        }

        public static Client FromSession(
            string projectId,
            string session,
            string endpoint = "https://cloud.appwrite.io/v1",
            string? endpointRealtime = null,
        ...
        public static Client FromImpersonation(
            string projectId,
            string session,
            string? userId = null,
            ...
            bool selfSigned = false)
        {
            var targetCount = new[] { userId, userEmail, userPhone }.Count(value => !string.IsNullOrEmpty(value));
            if (targetCount != 1)
            {
                throw new AppwriteException("Exactly one impersonation target must be provided.");
            }

            var client = FromSession(projectId, session, endpoint, endpointRealtime, locale, selfSigned);
            ...
            bool selfSigned)
        {
            ...
            client.SetHeader("project", "X-Appwrite-Project", projectId);
            ...
            client._selfSigned = selfSigned;

            if (!string.IsNullOrEmpty(locale))
            {
                client.SetHeader("locale", "X-Appwrite-Locale", locale);
            }
        }
        ...
        private static string ToRealtimeEndpoint(string endpoint)
        {
            return endpoint
                .Replace("http://", "ws://")
                .Replace("https://", "wss://");
            ...
            if (isMultipart)
            {
                ...
                foreach (var parameter in parameters)
                {
                    if (parameter.Value is InputFile inputFile)
                    {
                        ...
                        {
                            case "path":
                                if (string.IsNullOrEmpty(inputFile.Path) || !System.IO.File.Exists(inputFile.Path))
                                {
                                    throw new FileNotFoundException("Upload file not found.", inputFile.Path);
                                }
                            ...
                            case "stream":
                                ...
                                {
                                    throw new InvalidOperationException("Upload stream data is missing or invalid.");
                                }
                                ...
                                break;
                            case "bytes":
                                fileData = inputFile.Data as byte[] ?? throw new InvalidOperationException("Upload byte data is missing or invalid.");
                                break;
                            default:
                                throw new InvalidOperationException($"Unsupported upload source type: {inputFile.SourceType}");
                        }
            ...
            Func<Dictionary<string, object>, T>? convert = null) where T : class
        {
            ...
            if (responseHeaders != null)
            {
                ...
                _cookieContainer.ParseSetCookieHeaders(setCookieHeaders, uri.Host);
            }
#endif
        ...
        private static async UniTask<int> ReadExactlyAsync(Stream stream, byte[] buffer, int count)
        {
            var totalRead = 0;
            ...
            Action<UploadProgress>? onProgress = null) where T : class
        {
                ...
                if (size < ChunkSize)
                {
                    switch(input.SourceType)
                    {
                        ...
                        case "stream":
                            ...
                            if (dataStream == null)
                                throw new InvalidOperationException("Stream data is null");
                            var bytesReadSmall = await ReadExactlyAsync(dataStream, buffer, (int)size);
                            if (bytesReadSmall != size)
                                throw new EndOfStreamException("Unexpected end of stream during upload");
                            break;
                        case "bytes":
                ...
                while (offset < size)
                {
                    ...
                    switch(input.SourceType)
                    {
                        ...
                        case "stream":
                            ...
                            var requestedBytes = (int)Math.Min(size - offset, ChunkSize);
                            var bytesRead = await ReadExactlyAsync(stream, buffer, requestedBytes);
                            if (bytesRead != requestedBytes)
                                throw new EndOfStreamException("Unexpected end of stream during chunked upload");
                            ...
                            break;
                        case "bytes":
                            var sourceBytes = (byte[])input.Data;
                            var chunkLength = (int)Math.Min(size - offset, ChunkSize);
                            chunkData = new byte[chunkLength];
                            Buffer.BlockCopy(sourceBytes, (int)offset, chunkData, 0, chunkLength);
                            break;
                        default:
                    ...
                    {
                        ["Content-Type"] = "multipart/form-data",
                        ["Content-Range"] = $"bytes {offset}-{offset + chunkData.Length - 1}/{size}"
                    };
                    ...
                    onProgress?.Invoke(
                        new UploadProgress(
                            id: id,
                            progress: size == 0 ? 100 : (double)Math.Min(offset, size) / size * 100,
            ...
            {
                if (input.SourceType == "path" && input.Data is Stream fileStream)
                {
                    fileStream.Dispose();
                }
            }
