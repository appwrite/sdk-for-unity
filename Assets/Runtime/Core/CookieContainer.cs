...
namespace Appwrite
{
    ...
    public class Cookie
    {
        ...
        public bool IsExpired =>
            MaxAge.HasValue
                ? MaxAge <= 0 || DateTime.UtcNow > CreatedAt.ToUniversalTime().AddSeconds(MaxAge.Value)
                : Expires != DateTime.MaxValue && DateTime.UtcNow > Expires.ToUniversalTime();
    ...
    public class CookieContainer
    {
        ...
        public CookieContainer()
        {
            ...
            try
            {
                ...
                Debug.Log("[CookieContainer] WebGL credentials enabled - browser will handle cookies.");
            }
            catch
            {
                // Ignore errors - jslib may not be loaded
            }
            // In WebGL, the browser handles cookies automatically, so we don't load from PlayerPrefs
#else
            // In non-WebGL builds, load cookies from PlayerPrefs
            LoadCookies();
#endif
        }
        ...
        /// </summary>
        public void ParseSetCookieHeaders(IEnumerable<string> setCookieHeaders, string domain)
        {
            if (setCookieHeaders == null || string.IsNullOrWhiteSpace(domain)) return;

            foreach (var setCookieHeader in setCookieHeaders)
            {
                ParseSetCookieHeader(setCookieHeader, domain);
            }
        }
        ...
        /// <summary>
        /// Remove expired cookies (must be called within lock)
        /// </summary>
        ...
        public void LoadCookies()
        {
            lock (_lock)
            {
                try
                {
                    ...
                    {
                        var loaded = JsonSerializer.Deserialize<List<Cookie>>(PlayerPrefs.GetString(CookiePrefsKey), Client.DeserializerOptions);
                        _cookies.Clear();
                        if (loaded != null)
                        {
                            _cookies.AddRange(loaded);
                        }
                    }
        ...
        /// <summary>
        /// Save cookies to persistent storage (must be called within lock)
        /// </summary>
        private void SaveCookies()
        {
            // Note: This method should only be called from within a lock block
            try
            {
                CleanExpiredCookies();
