...
namespace Appwrite
{
    public static class WebAuthComponent
    {
        ...
        private static void Initialize()
        {
            ...
            Application.deepLinkActivated += OnDeepLinkActivated;
            Debug.Log("[AppwriteDeepLinkHandler] Initialized for OAuth callbacks.");
        }

        private static void OnDeepLinkActivated(string url)
        {
            Debug.Log($"[AppwriteDeepLinkHandler] Received deep link: {url}");
        ...
        public static async UniTask<Uri> Authenticate(string authUrl)
        {
            ...
            var queryDict = authUri.Query.FromQueryString();
            var projectId = queryDict.GetValueOrDefault("project");
            if (string.IsNullOrEmpty(projectId))
            {
                throw new AppwriteException("Project ID not found in authentication URL.");
            }

            var callbackScheme = $"appwrite-callback-{projectId}";
            var tcs = new UniTaskCompletionSource<Uri>();
            
            if (!PendingAuth.TryAdd(callbackScheme, tcs))
            {
                throw new AppwriteException("Authentication process already in progress.");
            }
